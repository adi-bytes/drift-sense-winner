...
_GENERATORS: dict = {"dram": generate_dram_canvas, "finfet": generate_finfet_canvas}

STRIP_BASE_VAL: int = 95
STRIP_LINE_VAL: int = 128
STRIP_LINE_PITCH_NM: float = 220
STRIP_LINE_WIDTH_NM: float = 9
...
def _strip_routing_texture(size_px: int, rng: np.random.Generator) -> np.ndarray:
    ...
    for axis_positions, is_row in (
        (
            np.arange(
                rng.uniform(0, STRIP_LINE_PITCH_NM), size_px, STRIP_LINE_PITCH_NM
            ),
            True,
        ),
        (
...
def generate_zone_canvas(
    ...
    mat_rects: list[tuple[int, int, int, int]] = []
    strip_rects: list[tuple[int, int, int, int]] = []
    ...
    for row_is_mat, y0, y1 in row_spans:
        for col_is_mat, x0, x1 in col_spans:
            if row_is_mat and col_is_mat and y1 > y0 and x1 > x0:
                ...
                preset = fixed_preset or presets[int(rng.integers(0, len(presets)))]
