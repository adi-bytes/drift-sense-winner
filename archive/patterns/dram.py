...
def _line_positions(
    size_px: int, pitch_nm: float, rng: np.random.Generator
) -> np.ndarray:
    """Generate line-center positions with slight pitch jitter."""
...
def _line_mask(
    ...
    widths = biased_width_nm * (
        1.0 + rng.normal(0, width_jitter_fraction, size=len(positions))
    )
        ...
        for j, bl in enumerate(bit_positions):
            if (i + j) % 2 == 0:
                radius = max(
                    ...
                    int(
                        round(
                            base_radius * (1.0 + rng.normal(0, WIDTH_JITTER_FRACTION))
                        )
                    ),
                )
                cv2.circle(
                    canvas, (int(round(bl)), int(round(wl))), radius, CONTACT_VAL, -1
                )
                cv2.circle(
                    contact_layer,
                    (int(round(bl)), int(round(wl))),
                    radius,
                    CONTACT_VAL,
                    -1,
                )
